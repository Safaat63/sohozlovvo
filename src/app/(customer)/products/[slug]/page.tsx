...
export default async function ProductDetailPage({
    ...
    return (
        <div className="min-h-screen bg-background pb-12 font-sans relative">
            <ScrollToTop />
            ...
            <div className="bg-background py-4">
                <div className="container mx-auto px-4 md:px-10">
                    <ul className="flex flex-wrap gap-2 items-center text-sm">
                        ...
                        <li className="text-foreground font-semibold">Products</li>
                    </ul>
                </div>
            </div>
            ...
            <div className="mx-auto px-4 md:px-10">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 bg-card p-5 md:p-8 rounded-xl border border-border shadow-sm">
                    ...
                    <div className="w-full flex flex-col pt-1">
                        <h1 className="text-card-foreground text-[22px] md:text-[28px] font-bold leading-tight mb-1">
                            {product.name}
                        </h1>
                        ...
                        {product.brand && (
                            <div className="mt-8 border border-border rounded px-4 py-2 inline-flex items-center gap-3 w-max">
                                ...
                                <span className="font-bold text-lg text-foreground tracking-wide uppercase">{product.brand}</span>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="mt-8 bg-card rounded-xl border border-border shadow-sm relative">
                    ...
                    <div className="p-6 md:p-8">
                        ...
                        <div id="description" className="mb-12 scroll-mt-24">
                            ...
                            {product.specifications && product.specifications.length > 0 && (
                                <div className="mt-8 bg-muted/50 rounded border border-border p-5">
                                    <h4 className="font-bold text-card-foreground mb-4">Specifications</h4>
                                    <div className="flex flex-col">
                                        {product.specifications.map((spec: any, idx: number) => (
                                            <div key={spec.id} className={`flex py-2 px-4 ${idx % 2 === 0 ? 'bg-card' : ''} border-b border-border last:border-b-0`}>
                                                <span className="text-card-foreground font-medium w-1/3">{spec.key}</span>
                        ...
                        {/* Video Section */}
                        {videoUrl && (
                            <div id="video" className="mb-12 pt-8 border-t border-border scroll-mt-24">
                                <h3 className="text-[18px] font-bold text-card-foreground mb-5 inline-block border-b-[3px] border-primary pb-1">
                                    Video
                                </h3>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
                                ...
                                <div className="lg:col-span-4">
                                    <div className="flex items-center gap-4 mb-3">
                                        ...
                                        <div>
                                            <p className="text-card-foreground font-medium text-sm mb-1">Average Rating</p>
                                            <div className="flex gap-1 text-muted-foreground text-sm">
                                                {Array.from({ length: 5 }).map((_, i) => (
                                                    <Star key={i} className={`w-4 h-4 ${i < Math.floor(productRating) ? "fill-primary text-primary" : ""}`} />
                                                ))}
                                                <span className="ml-2">({verifiedReviews.length} Reviews)</span>
                                            </div>
                                        </div>
                                    </div>
                                    <p className="text-sm font-bold text-card-foreground mb-6">0.00% <span className="font-normal text-muted-foreground">Recommended (0 of 0)</span></p>
                                        ...
                                        {[5, 4, 3, 2, 1].map((stars) => (
                                            <div key={stars} className="flex items-center gap-3">
                                                <div className="flex gap-0.5">
                                                    {Array.from({ length: 5 }).map((_, i) => (
                                                        <Star key={i} className={`w-3 h-3 ${i < stars ? "fill-primary text-primary" : "fill-muted text-muted-foreground/30"}`} />
                                                    ))}
                                                </div>
                                ...
                                <div className="lg:col-span-8">
                                    {session?.user ? (
                                        <div className="mb-10">
                                            ...
                                            <p className="text-sm text-muted-foreground mb-5">Your email address will not be published. Required fields are marked *</p>
                                        ...
                                        <div className="mb-10 p-6 bg-muted border border-border rounded text-center">
                                            ...
                                            <Link href="/auth/login" className="inline-block py-2 px-6 bg-primary text-primary-foreground font-bold rounded hover:bg-primary/90 transition-colors text-sm">
                                                Sign In
                                            </Link>
                                        </div>
                                    )}
                                    ...
                                    <div className="space-y-4">
                                        {verifiedReviews.length > 0 ? (
                                            verifiedReviews.map((review) => (
                                                <div key={review.id} className="py-4 border-b border-border last:border-0">
                                                    <div className="flex items-center gap-2 mb-2">
                                                        <div className="flex gap-0.5">
                                                            {Array.from({ length: 5 }).map((_, i) => (
                                                                <Star key={i} className={`w-3.5 h-3.5 ${i < Number(review.rating) ? "fill-primary text-primary" : "fill-muted text-muted-foreground/30"}`} />
                                                            ))}
                                                        </div>
                                                        <span className="font-bold text-sm text-card-foreground ml-2">{review.user.name}</span>
                                                        <span className="text-xs text-muted-foreground">• {formatDateDhaka(review.createdAt, "PP")}</span>
                                                    </div>
                                                    {review.title && <h5 className="font-bold text-card-foreground mb-1 text-sm">{review.title}</h5>}
                ...
                <div className="py-12 mt-4">
                    <div className="flex items-center justify-between mb-6 border-b border-border pb-3">
                        <h2 className="text-xl font-bold text-card-foreground uppercase">Related Products</h2>
                    </div>
