  ...
  return (
    <div className="space-y-0">
      ...
      <div className="py-4 border-b border-border">
        <h4 className="font-semibold text-sm mb-3 text-foreground uppercase tracking-wide">
          Category
        </h4>
        <ul className="space-y-2">
          <li>
            ...
            >
              <span
                className={`text-sm transition-colors ${!params.category ? "text-foreground font-medium" : "text-muted-foreground group-hover:text-primary"}`}
              >
                All Categories
              </span>
            </Link>
          ...
          {categories.map((category: Category) => (
            <li key={category.id}>
              ...
              >
                <span
                  className={`text-sm transition-colors ${params.category === category.slug ? "text-foreground font-medium" : "text-muted-foreground group-hover:text-primary"}`}
                >
                  {category.name}
                </span>
          ...
          <ul className="space-y-2">
            {brands.map((brand: string) => (
              <li key={brand}>
                ...
                >
                  <div
                    className={`w-4 h-4 rounded border ${params.brand === brand ? "bg-primary border-primary" : "border-border"} flex items-center justify-center`}
                  >
                    {params.brand === brand && (
                      <svg
                  ...
                  <span
                    className={`text-sm transition-colors ${params.brand === brand ? "text-foreground font-medium" : "text-muted-foreground group-hover:text-primary"}`}
                  >
                    {brand}
                  </span>
                </Link>
      ...
      <div className="py-4 border-b border-border">
        ...
        >
          <div
            className={`w-4 h-4 rounded border ${params.inStock === "true" ? "bg-primary border-primary" : "border-border"} flex items-center justify-center`}
          >
            {params.inStock === "true" && (
              <svg
                className="w-3 h-3 text-white"
          ...
          <span
            className={`text-sm transition-colors ${params.inStock === "true" ? "text-foreground font-medium" : "text-muted-foreground group-hover:text-primary"}`}
          >
            In Stock Only
          </span>
        </Link>
        ...
        <div className="space-y-2">
          {[4, 3, 2].map((rating) => (
            <Link
              ...
              href={buildQueryString({
                rating:
                  params.rating === String(rating) ? undefined : String(rating),
              })}
              className="flex items-center gap-3 cursor-pointer"
            >
              <div
                className={`w-4 h-4 rounded-full border ${params.rating === String(rating) ? "border-primary bg-primary" : "border-border"} flex items-center justify-center`}
              >
        ...
        <aside className="w-full lg:w-72 shrink-0">
          ...
          <div className="lg:hidden mb-4">
            <Sheet>
              ...
              <SheetContent
                side="left"
                className="w-full max-w-xs sm:max-w-sm overflow-y-auto p-6 bg-card"
              >
                <SheetHeader>
